...
{
    vec3_t plane_point = frustum_planes[plane].point; // 0, 0, 0
    vec3_t plane_normal = frustum_planes[plane].normal; // normal vector
    
    vec3_t inside_vertices[MAX_NUM_POLY_VERTICES]; // array where we'll store inside vertices
    tex2_t inside_texcoords[MAX_NUM_POLY_VERTICES]; // array where we'll store inside vertices
    int num_inside_vertices = 0; // counter
    
    // setting first iteration
    vec3_t* current_vertex = &polygon->vertices[0]; // first position pointer
    tex2_t* current_texcoord = &polygon->texcoords[0]; 
    
    vec3_t* previous_vertex = &polygon->vertices[polygon->num_vertices-1]; // last vertex cause circular
    tex2_t* previous_texcoord = &polygon->texcoords[polygon->num_vertices-1];
    
    float current_dot = 0; // settng dot product variables
    ...
    while(current_vertex != &polygon->vertices[polygon->num_vertices])
    {
        ...
        // testing if we change from inside<->outside
        if (current_dot*previous_dot < 0)
        {   
            // find interpolation factor t = dotQ1 / (dotQ2 - dotQ1)
            float t = previous_dot / (previous_dot - current_dot);
            // find the intersection I = Q1 + t(Q2-Q1)
            ...
            // find interpolated texcoords with lerp formula and 't'
            ...
            // inserting intersection point into "inside vertices"
            inside_vertices[num_inside_vertices] = vec3_clone(&intersection_point);
            inside_texcoords[num_inside_vertices] = tex2_clone(&interpolated_texcoord);
            num_inside_vertices++;
        }
        
        // if vertex is "inside vertices"
        if (current_dot > 0)
        {
            // Insert the current vertex to the list of "inside vertices" same as before
            inside_vertices[num_inside_vertices] = vec3_clone(current_vertex);
            inside_texcoords[num_inside_vertices] = tex2_clone(current_texcoord);
            num_inside_vertices++;
        }
        
        // setting previous as current and current to next
    ...
    // At the end, update polygon with updated vertices (out parameter)
